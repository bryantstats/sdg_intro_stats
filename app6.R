# app.R — Full version with Regression Tab added
...
server <- function(input, output, session) {
  ...
  output$trendplot <- renderPlotly({
    dat <- trend_data()
    validate(need(nrow(dat) > 0, "No data available for selected inputs."))
  ...
  output$tbl_trend <- renderDT({
    dat <- trend_data()
    validate(need(nrow(dat) > 0, "No data to display."))
